...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if args.dataset_name is not None:
            ...
            """
            Loading all imagers. This code is not native. Use at your own risk.
            """
            instance_images = []
            supported_fmts_img = ".jpeg", ".jpg", ".png"
            for path in list(Path(instance_data_root).iterdir()):
                tmp = str(path)
                if any(sfmt in tmp for sfmt in supported_fmts_img):
                    instance_images.append(Image.open(path))
            print(f"length: {len(instance_images)}")

            supported_fmts_txt = [".txt"]
            self.custom_instance_prompts = []
            for path in list(Path(instance_data_root).iterdir()):
                tmp = str(path)
                if any(sfmt in tmp for sfmt in supported_fmts_txt):
                    cap_file = open(tmp, "r")
                    retrieved_caption = cap_file.read()
                    cap_file.close()
                    self.custom_instance_prompts.append(retrieved_caption)

            # print(f"prompts: {self.custom_instance_prompts}")
...
def encode_prompt(
    ...
    clip_prompt_embeds = torch.nn.functional.pad(
        clip_prompt_embeds, (0,
                             t5_prompt_embed.shape[-1] - clip_prompt_embeds.shape[-1])
    )
...
def main(args):
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            has_supported_fp16_accelerator = torch.cuda.is_available(
    ...
    if not train_dataset.custom_instance_prompts:
        ...
        # batch prompts on all training steps
            ...
            tokens_three = tokenize_prompt(
                tokenizer_three, args.instance_prompt)
            if args.with_prior_preservation:
                class_tokens_one = tokenize_prompt(
                    tokenizer_one, args.class_prompt)
                class_tokens_two = tokenize_prompt(
                    tokenizer_two, args.class_prompt)
                class_tokens_three = tokenize_prompt(
                    tokenizer_three, args.class_prompt)
                tokens_one = torch.cat([tokens_one, class_tokens_one], dim=0)
                tokens_two = torch.cat([tokens_two, class_tokens_two], dim=0)
                tokens_three = torch.cat(
                    [tokens_three, class_tokens_three], dim=0)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if args.weighting_scheme == "logit_normal":
                    ...
                    u = torch.normal(
                        mean=args.logit_mean, std=args.logit_std, size=(bsz,), device="cpu")
                    u = torch.nn.functional.sigmoid(u)
                elif args.weighting_scheme == "mode":
                    u = torch.rand(size=(bsz,), device="cpu")
                    u = 1 - u - args.mode_scale * \
                ...
                noisy_model_input = sigmas * noise + \
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                pipeline = StableDiffusion3Pipeline.from_pretrained(
                    ...
                    text_encoder_3=accelerator.unwrap_model(
                        text_encoder_three),
...
## from https://github.com/huggingface/diffusers/blob/main/examples/dreambooth/train_dreambooth_lora_sd3.py
## blog: https://huggingface.co/blog/sd3
# accelerate launch imagen\train_dreambooth_lora_sd3.py \
#   --pretrained_model_name_or_path=stabilityai/stable-diffusion-3-medium-diffusers \
#   --dataset_name=wait-whoami/ImageCLEFmed-MEDVQA-GI-2024-Dev_mod \
#   --instance_prompt "XXX" \
...
#   --validation_prompt="Generate an image with an polyp located in the center-right." \
#   --validation_epochs=1 \
#   --lr_warmup_steps=0   \
#   --seed=0 \
#   --rank 128 \
...
#   --output_dir=MEDVQA_SD3_db_lora_sigma_sqrt

# https://blog.segmind.com/dreambooth-lora-understanding-fine-tuning-parameters/
